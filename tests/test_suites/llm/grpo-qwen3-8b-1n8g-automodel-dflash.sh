...
if [[ $(jq 'to_entries | .[] | select(.key == "train/loss") | .value | keys | map(tonumber) | max' $JSON_METRICS) -ge $MAX_STEPS ]]; then
    # Metric scales anchored to the 1n8g smoke run: draft_loss (CE+TV,
    # summed over microbatches) started ~264 and declined; confidence loss is
    # exactly 0 for dflash; acceptance length was 3.1-3.5 from step 1 and
    # rose with co-training.
    uv run tests/check_metrics.py $JSON_METRICS \
        'min(data["train/draft_loss"]) > 0' \
        'data["train/draft_loss"]["30"] < 300' \
